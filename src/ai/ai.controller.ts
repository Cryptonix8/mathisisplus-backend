...
export class AiController {
  ...
  @UseInterceptors(
    FileInterceptor('audio', {
      ...
      fileFilter: (req, file, cb) => {
        ...
        const hasAudioExt = /\.(m4a|mp4|mp3|wav|webm|ogg|caf)$/i.test(file.originalname || '');
        if (allowedMimes.includes(file.mimetype) || isAudioMime || hasAudioExt) {
          cb(null, true);
        } else {
  ...
  async voiceChat(
    ...
    @Body('learningMode') learningMode?: 'hints' | 'full_solution',
  ) {
    if (!file) {
