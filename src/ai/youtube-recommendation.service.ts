...
export class YouTubeRecommendationService {
  ...
  async getConfig() {
    ...
    return {
      ...
      requireGreek: dbConfig?.requireGreek ?? true,
  ...
  async recommend(params: RecommendParams) {
    ...
    if (!apiKey) {
      return {
        ...
        quality: {
          weak: true,
          reason:
            'Οι προτάσεις βίντεο δεν είναι διαθέσιμες προσωρινά (λείπει YouTube API key).',
        },
      };
    }
    ...
    if (uniqueMap.size < 3) {
      const fallbackQueries = [
        { q: `${params.topic} εξήγηση`, relevanceLanguage: 'el' as string | undefined },
        { q: `${params.topic} παραδείγματα`, relevanceLanguage: 'el' as string | undefined },
      ...
      return {
        ...
        quality: {
          ...
          reason: clarificationQuestion
            ...
            : 'Δεν βρέθηκαν κατάλληλα βίντεο. Δοκίμασε πιο συγκεκριμένο θέμα (μάθημα + κεφάλαιο).',
    ...
    return {
      ...
      quality: weak
        ? {
            ...
            reason: clarificationQuestion
              ? clarificationQuestion
              : 'Τα αποτελέσματα είναι περιορισμένα. Πρόσθεσε τάξη και πιο συγκεκριμένο κεφάλαιο/θέμα.',
            needsClarification: Boolean(clarificationQuestion),
          }
        : { weak: false },
    };
  ...
  private score(params: {
    ...
    const hasGreek = this.looksGreek(`${params.title} ${params.description}`);
    if (hasGreek) score += 3;
    else if (params.requireGreek) score -= 6;
  ...
  private extractIntentContextTokens(topic: string, subject?: string, locale?: string) {
    ...
    if (languageIntent.some((k) => text.includes(this.normalizeText(k)))) {
      tokens.push('γραμματική', 'γλώσσα', 'μέρος του λόγου', 'ελληνική γλώσσα');
    }

    if (mathIntent.some((k) => text.includes(this.normalizeText(k)))) {
      tokens.push('μαθηματικά', 'άσκηση', 'βήμα βήμα');
    }

    if (locale === 'el-GR' && tokens.length === 0) {
      tokens.push('σχολικό μάθημα');
    }

    return tokens;
  }

  private looksGreek(value: string) {
  ...
  private buildSearchQueries(params: {
    ...
    const hasGrammarIntent = params.intentContextTokens.some((t) =>
      this.normalizeText(t).includes('γραμματικ'),
    );

    const queries = [
      { q: params.query, relevanceLanguage: 'el' as string | undefined },
      { q: params.query, relevanceLanguage: undefined as string | undefined },
      {
        q: [params.subject, params.topic].filter(Boolean).join(' '),
        relevanceLanguage: params.requireGreek ? ('el' as string | undefined) : undefined,
      },
    ];

    if (hasGrammarIntent && concept) {
      queries.push(
        { q: `${concept} γραμματική`, relevanceLanguage: 'el' as string | undefined },
        { q: `${concept} μέρος του λόγου`, relevanceLanguage: 'el' as string | undefined },
        { q: `${concept} ελληνική γλώσσα`, relevanceLanguage: 'el' as string | undefined },
      );
    }
  ...
  private getClarificationQuestion(
    ...
    const ambiguousTermPrompts: Array<{ terms: string[]; question: string }> = [
      {
        terms: ['συνδεσμος', 'συνδεσμοι'],
        question:
          'Εννοείς τον "σύνδεσμο" στη Γραμματική (μέρος του λόγου) ή "σύνδεσμο" ως URL/ιστοσελίδα;',
      },
      {
        terms: ['οριο', 'ορια'],
        question:
          'Εννοείς "όρια" στα Μαθηματικά ή "όρια" με γενική έννοια; Σε ποιο μάθημα το θες;',
      },
      {
        terms: ['πινακας', 'πινακες'],
        question:
          'Εννοείς πίνακες στη Στατιστική/Μαθηματικά ή πίνακες δεδομένων (π.χ. υπολογιστικά φύλλα);',
      },
      {
        terms: ['συναρτηση', 'συναρτησεις'],
        question:
          'Εννοείς "συνάρτηση" στα Μαθηματικά; Αν ναι, ποιο κομμάτι (ορισμός, γραφική παράσταση, πράξεις);',
      },
    ];
    ...
    if (coreTokens.length <= 1 && intentContextTokens.length === 0) {
      return 'Μπορείς να μου πεις μάθημα και κεφάλαιο για να σου προτείνω σχετικά βίντεο;';
    }
