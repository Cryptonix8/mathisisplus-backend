...
export class AutoInitService implements OnModuleInit {
  ...
  constructor(
    ...
    private greekCurriculumSeed: GreekCurriculumSeedService,
  ) {}
  ...
  private async initializeInBackground() {
    ...
    try {
      ...
      // Step 2b: Greek curriculum — subjects + units (κεφάλαια) per grade (el-GR)
      const needsGreekSeed =
        (await this.greekCurriculumSeed.needsGreekSubjectSeed()) ||
        (await this.greekCurriculumSeed.needsGreekTopicSeed());
      if (needsGreekSeed) {
        this.logger.log('🇬🇷 Seeding Greek year groups, subjects, and curriculum units...');
        const greekResult = await this.greekCurriculumSeed.seedGreekCurriculum();
        this.logger.log(
          `✅ Greek seed: ${greekResult.subjectsCreated} subjects created, ` +
            `${greekResult.subjectsExisting} updated, ${greekResult.skillsCreated} skills, ` +
            `${greekResult.topicsCreated} units created, ${greekResult.topicsExisting} units updated`,
        );
      } else {
        this.logger.log('✅ Greek curriculum subjects and units already complete');
      }
  ...
  private async createDefaultUsers() {
    ...
    const adminExists = await this.prisma.user.findUnique({
      where: { email: 'admin@mathisisplus.gr' },
    });

    if (!adminExists) {
      await this.prisma.user.create({
        data: {
          email: 'admin@mathisisplus.gr',
    ...
    const teacherExists = await this.prisma.user.findUnique({
      where: { email: 'teacher@mathisisplus.gr' },
    });

    if (!teacherExists) {
      await this.prisma.user.create({
        data: {
          email: 'teacher@mathisisplus.gr',
          password: hashedPassword,
          firstName: 'Demo',
          lastName: 'Teacher',
          role: 'TEACHER',
          ...
          try {
            await this.prisma.feedbackTest.create({
              data: {
                subjectId: subject.id,
                skillId: skill.id,
                title: `Αυτοαξιολόγηση – ${skill.displayName}`,
                description: `Αξιολόγησε τη σιγουριά σου στις δεξιότητες «${skill.displayName}».`,
                isActive: true,
                questions: {
                  create: [
                    { statement: `Κατανοώ τις βασικές έννοιες στο «${skill.displayName}».`, orderIndex: 0 },
                    { statement: `Μπορώ να εφαρμόσω δεξιότητες στο «${skill.displayName}» σε απλές ασκήσεις.`, orderIndex: 1 },
                    { statement: `Μπορώ να αντιμετωπίσω απαιτητικές ασκήσεις στο «${skill.displayName}».`, orderIndex: 2 },
                    { statement: `Μπορώ να εξηγώ τις έννοιες του «${skill.displayName}» σε άλλους.`, orderIndex: 3 },
                  ],
                },
              },
            });
