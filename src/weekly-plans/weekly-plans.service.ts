...
export class WeeklyPlansService {
  ...
  /** Build subject locale filter for Prisma where (en-GB includes null). */
  private subjectLocaleWhere(locale: string | null | undefined) {
    const effective = locale === 'el-GR' ? 'el-GR' : 'en-GB';
    return effective === 'el-GR'
      ? { locale: 'el-GR' as const }
      : { OR: [{ locale: 'en-GB' }, { locale: null }] };
  }
   ...
   * Returns all subjects from the student's year group matching the year group's locale (Greek or English).
   */
