...
import { normalizeElGROptionLabel, normalizeElGRQuestionnaireSections } from '../common/greek-onboarding-text';
...
export class QuestionnaireParserService {
        ...
        if (txtPath) {
          try {
            ...
            if (questionnaire.sections && questionnaire.sections.length > 0) {
              questionnaire.sections = normalizeElGRQuestionnaireSections(questionnaire.sections);
  ...
  private parseQuestionnaire(content: string, yearNumber: number, locale?: string): any {
    ...
    const normalizedSections =
      effectiveLocale === 'el-GR' ? normalizeElGRQuestionnaireSections(sections) : sections;
    return { year: yearNumber, sections: normalizedSections };
  }
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (hasCheckbox && currentQuestion && currentSection) {
        ...
        parts.forEach((label, idx) => {
          const normalizedLabel = normalizeElGROptionLabel(label);
          currentQuestion.options.push({
            value: slug(normalizedLabel, currentQuestion.options.length + idx),
