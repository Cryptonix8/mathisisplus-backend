...
import {
  GREEK_DEFAULT_DIAGNOSTIC_BASE,
  GREEK_DIAGNOSTIC_GENERIC,
  diagnosticExplanationElGR,
  normalizeElGRQuestionnaireSections,
  onboardingQuestionsLookEnglish,
} from '../common/greek-onboarding-text';
...
export class OnboardingTestsService {
  ...
  private diagnosticQuestionsCache = new Map<string, { questions: any[]; testedYearGroup: string }>();
  private diagnosticQuestionsInFlight = new Map<string, Promise<{ questions: any[]; testedYearGroup: string }>>();

  /**
   * Greek app sends locale=el-GR but year rows may be en-GB-seeded. Prefer the client's curriculum
   * locale when choosing questionnaire/diagnostic files and Part A titles.
   */
  private resolveContentLocale(preferredLocale?: string | null, yearGroupLocale?: string | null): string {
    if (preferredLocale === 'el-GR') return 'el-GR';
    if (preferredLocale === 'en-GB') return 'en-GB';
    return yearGroupLocale || 'en-GB';
  }
  ...
  async generatePersonalityTest(userId: string, preferredLocale?: string) {
    ...
    // If no sections found (file missing or empty), use fallback questions (locale-aware for Greek)
    ...
    if (locale === 'el-GR' && sections.length > 0) {
      sections = normalizeElGRQuestionnaireSections(sections);
    }
  ...
  /**
   * Fallback personality questions when questionnaire file is not available.
   * Returns Greek questions when locale is el-GR, otherwise English.
   */
  private getFallbackPersonalityQuestions(yearNumber: number, locale?: string): any[] {
    const effectiveLocale = locale === 'el-GR' ? 'el-GR' : 'en-GB';
    if (effectiveLocale === 'el-GR') {
      return this.getFallbackPersonalityQuestionsElGR();
    }
    return [
      {
        title: 'About You',
  ...
  /**
   * Greek fallback personality questions (same structure and value keys as English for answer mapping)
   */
  private getFallbackPersonalityQuestionsElGR(): any[] {
    return [
      {
        ...
        questions: [
          {
            id: 'languages',
            question: 'Ποιες γλώσσες μιλάτε στο σπίτι; (Επιλέξτε όλες όσες ισχύουν)',
            type: 'multiple_select',
            options: [
              { value: 'english', label: 'Αγγλικά' },
              { value: 'chinese', label: 'Κινεζικά' },
              { value: 'hindi', label: 'Χίντι' },
              { value: 'greek', label: 'Ελληνικά' },
              { value: 'albanian', label: 'Αλβανικά' },
              { value: 'arabic', label: 'Αραβικά' },
              { value: 'russian', label: 'Ρωσικά' },
              { value: 'other', label: 'Άλλο' },
            ],
          },
          {
            id: 'confidence',
            question: 'Πόσο σίγουροι αισθάνεστε για τη μάθησή σας φέτος;',
            type: 'multiple_choice',
            options: [
              { value: 'very_confident', label: 'Πολύ σίγουροι' },
              { value: 'quite_confident', label: 'Αρκετά σίγουροι' },
              { value: 'a_bit_unsure', label: 'Λίγο αβέβαιοι' },
              { value: 'not_confident', label: 'Όχι σίγουροι' },
            ],
          },
        ],
      },
      {
        ...
        questions: [
          {
            id: 'learning_style',
            question: 'Πώς προτιμάτε να μαθαίνετε νέα πράγματα; (Επιλέξτε έως 3)',
            type: 'multiple_select',
            options: [
              { value: 'watching_videos', label: 'Βλέποντας βίντεο' },
              { value: 'reading_texts', label: 'Διαβάζοντας κείμενα' },
              { value: 'hands_on_activities', label: 'Πρακτικές δραστηριότητες' },
              { value: 'listening_explanations', label: 'Ακούγοντας εξηγήσεις' },
              { value: 'working_with_others', label: 'Συνεργαζόμενοι με άλλους' },
              { value: 'working_alone', label: 'Δουλεύοντας μόνοι σας' },
            ],
          },
          {
            id: 'difficulty_help',
            question: 'Τι σας βοηθά να κατανοήσετε κάτι δύσκολο;',
            type: 'multiple_select',
            options: [
              { value: 'step_by_step_examples', label: 'Παραδείγματα βήμα-βήμα' },
              { value: 'pictures_diagrams', label: 'Εικόνες και διαγράμματα' },
              { value: 'trying_myself', label: 'Δοκιμάζοντας μόνοι σας' },
              { value: 'hearing_explanations', label: 'Ακούγοντας εξηγήσεις' },
              { value: 'practice_questions', label: 'Ερωτήσεις εξάσκησης' },
            ],
          },
          {
            id: 'difficulty_response',
            question: 'Όταν δεν κατανοείτε κάτι, τι προτιμάτε;',
            type: 'multiple_choice',
            options: [
              { value: 'try_again_alone', label: 'Να δοκιμάσω ξανά μόνος μου' },
              { value: 'get_hint', label: 'Να πάρω μια υπόδειξη' },
              { value: 'see_another_example', label: 'Να δω άλλο παράδειγμα' },
              { value: 'get_full_explanation', label: 'Να πάρω πλήρη εξήγηση' },
              { value: 'ask_for_help', label: 'Να ζητήσω βοήθεια' },
            ],
          },
        ],
      },
      {
        ...
        questions: [
          {
            id: 'preferred_subjects',
            question: 'Ποια μαθήματα σας αρέσουν περισσότερο; (Επιλέξτε έως 3)',
            type: 'multiple_select',
            options: [
              { value: 'english', label: 'Αγγλικά' },
              { value: 'maths', label: 'Μαθηματικά' },
              { value: 'science', label: 'Επιστήμες' },
              { value: 'history', label: 'Ιστορία' },
              { value: 'geography', label: 'Γεωγραφία' },
              { value: 'art', label: 'Τέχνη' },
              { value: 'music', label: 'Μουσική' },
              { value: 'pe', label: 'Α.Φ.Μ.' },
              { value: 'computing', label: 'Πληροφορική' },
            ],
          },
          {
            id: 'difficult_subjects',
            question: 'Ποια μαθήματα σας φαίνονται λίγο δύσκολα; (Επιλέξτε έως 3)',
            type: 'multiple_select',
            options: [
              { value: 'english', label: 'Αγγλικά' },
              { value: 'maths', label: 'Μαθηματικά' },
      ...
      {
        title: 'Προτιμήσεις μελέτης',
        description: '',
        questions: [
          {
            id: 'preferred_time',
            question: 'Πότε προτιμάτε να κάνετε εργασίες Supercurriculum;',
            type: 'multiple_choice',
            options: [
              { value: 'after_school', label: 'Μετά το σχολείο' },
              { value: 'evening', label: 'Το απόγευμα' },
              { value: 'weekends', label: 'Σαββατοκύριακα' },
              { value: 'little_every_day', label: 'Λίγο κάθε μέρα' },
              { value: 'when_have_time', label: 'Μόνο όταν έχω χρόνο' },
            ],
          },
          {
            id: 'study_duration',
            question: 'Πόσο καιρό μπορείτε να μελετάτε χωρίς να κουραστείτε;',
            type: 'multiple_choice',
            options: [
              { value: '5_minutes', label: '5 λεπτά' },
              { value: '10_minutes', label: '10 λεπτά' },
              { value: '15_minutes', label: '15 λεπτά' },
              { value: '20_minutes', label: '20 λεπτά' },
              { value: 'more_than_20', label: 'Περισσότερα από 20 λεπτά' },
            ],
          },
          {
            id: 'task_preference',
            question: 'Τι τύπο εργασιών απολαμβάνετε περισσότερο;',
            type: 'multiple_select',
            options: [
              { value: 'creative_activities', label: 'Δημιουργικές δραστηριότητες' },
              { value: 'reading_tasks', label: 'Εργασίες ανάγνωσης' },
              { value: 'problem_solving', label: 'Εργασίες επίλυσης προβλημάτων' },
              { value: 'experiments', label: 'Πειράματα' },
              { value: 'quizzes_challenges', label: 'Κουίζ και προκλήσεις' },
            ],
          },
        ],
      },
   ...
   * en-GB: "Year 7" -> 7. el-GR: gymnasio_1 -> 7, dimotiko_1 -> 1, lykeio_1 -> 10, etc.
   */
  private getLogicalYearNumber(yearGroup: {
    ...
    const name = (yearGroup.name || '').toLowerCase();
    const display = (yearGroup.displayName || '').toLowerCase();
    const locale = yearGroup.locale || 'en-GB';

    // Prefer explicit year encodings from name/displayName.
    // This avoids relying on DB orderIndex when it has global sequencing values.
    const yearNameMatch = name.match(/year[_\s-]?(\d{1,2})/);
    if (yearNameMatch) return parseInt(yearNameMatch[1], 10);

    const yearDisplayMatch = display.match(/year\s*(\d{1,2})/);
    if (yearDisplayMatch) return parseInt(yearDisplayMatch[1], 10);

    // Greek curriculum naming support.
    const dimotikoName = name.match(/dimotiko[_\s-]?(\d{1,2})/);
    if (dimotikoName) return parseInt(dimotikoName[1], 10);
    const gymnasioName = name.match(/gymnasio[_\s-]?(\d{1,2})/);
    if (gymnasioName) return 6 + parseInt(gymnasioName[1], 10);
    const lykeioName = name.match(/lykeio[_\s-]?(\d{1,2})/);
    if (lykeioName) return 9 + parseInt(lykeioName[1], 10);

    // Fallback: first number in displayName (best effort).
    const yearMatch = yearGroup.displayName.match(/\d+/);
    if (yearMatch) return parseInt(yearMatch[0], 10);

    // Last fallback for legacy data without structured names.
    if (typeof yearGroup.orderIndex === 'number' && Number.isFinite(yearGroup.orderIndex)) {
      return yearGroup.orderIndex;
    }

    return locale === 'el-GR' ? 1 : 5;
  }

  /**
   * Get the minimum (first) year group for a locale (en-GB or el-GR).
  ...
  async generateDiagnosticTest(userId: string, currentYearGroupId: string, preferredLocale?: string) {
    ...
    const minYearNum = minimumYear ? this.getLogicalYearNumber(minimumYear as any) : (locale === 'el-GR' ? 1 : 5);
    ...
    // This handles cases where a test was created before the fix (en-GB: Year 5, el-GR: dimotiko_1)
   ...
   * Uses Diagnostic tests.pdf or (el-GR) Didaktiko-Paketo-K{N}-LEARNER for the previous year.
   */
  private async generateDiagnosticQuestions(yearGroup: any, subjects: any[], contentLocale?: string) {
    ...
    const cacheKey = `${locale}:${previousYearNum}`;
    this.logger.log(`Loading diagnostic test questions for Year ${previousYearNum} (locale: ${locale})`);

    const fromCache = this.diagnosticQuestionsCache.get(cacheKey);
    if (fromCache) {
      return {
        questions: fromCache.questions.map((q: any) => ({ ...q })),
        testedYearGroup: fromCache.testedYearGroup,
      };
    }

    const inFlight = this.diagnosticQuestionsInFlight.get(cacheKey);
    if (inFlight) {
      const shared = await inFlight;
      return {
        questions: shared.questions.map((q: any) => ({ ...q })),
        testedYearGroup: shared.testedYearGroup,
      };
    }

    const generator = this.buildDiagnosticQuestions(previousYearNum, yearGroup, subjects, locale);
    this.diagnosticQuestionsInFlight.set(cacheKey, generator);

    try {
      const generated = await generator;
      this.diagnosticQuestionsCache.set(cacheKey, {
        questions: generated.questions.map((q: any) => ({ ...q })),
        testedYearGroup: generated.testedYearGroup,
      });
      return {
        questions: generated.questions.map((q: any) => ({ ...q })),
        testedYearGroup: generated.testedYearGroup,
      ...
      this.diagnosticQuestionsInFlight.delete(cacheKey);
    }
  }

  private async buildDiagnosticQuestions(
    previousYearNum: number,
    yearGroup: any,
    subjects: any[],
    locale: string,
  ): Promise<{ questions: any[]; testedYearGroup: string }> {
    const allowOpenAiFallback = this.config.get<string>('ONBOARDING_OPENAI_FALLBACK_ENABLED') === 'true';
    try {
      ...
      if (!diagnosticTest || !diagnosticTest.questions || diagnosticTest.questions.length === 0) {
        this.logger.warn(`No diagnostic questions found for Year ${previousYearNum}, trying fallback generation`);
        if (allowOpenAiFallback && this.openai) {
          try {
          ...
          this.logger.warn(`OpenAI fallback is disabled (ONBOARDING_OPENAI_FALLBACK_ENABLED!=true); using static fallback`);
        }
        const fallback = this.getFallbackDiagnosticQuestions(yearGroup, subjects, locale);
        return {
      ...
      if (locale === 'el-GR' && onboardingQuestionsLookEnglish(questions)) {
        this.logger.warn(
          `Part B: Diagnostic file for Year ${previousYearNum} appears to be in English; using Greek fallback questions.`,
        );
        const fallback = this.getFallbackDiagnosticQuestions(yearGroup, subjects, locale);
        return {
          questions: fallback.questions,
      ...
      const formattedQuestions = questions.map((q: any, index: number) => ({
        ...
        explanation:
          locale === 'el-GR'
            ? diagnosticExplanationElGR(previousYearNum)
            : `This question tests prior knowledge from Year ${previousYearNum}`,
      }));

      const uniqueQuestions = this.deduplicateQuestionsById(formattedQuestions);
      this.logger.log(`Successfully loaded ${uniqueQuestions.length} diagnostic questions from file`);
      return {
  ...
  private async generateDiagnosticQuestionsWithOpenAI(
    ...
    const previousYearNum = this.getLogicalYearNumber(yearGroup);
    const locale = localeOverride ?? ((yearGroup as any).locale || 'en-GB');
    const lang = locale === 'el-GR' ? 'Greek' : 'English';
    const subjectNames = subjects.map((s) => s.displayName || s.name || '').filter(Boolean).join(', ') || 'general curriculum';
    const yearLabel = locale === 'el-GR'
      ? (['1η Δημοτικού', '2η Δημοτικού', '3η Δημοτικού', '4η Δημοτικού', '5η Δημοτικού', '6η Δημοτικού', '1η Γυμνασίου', '2η Γυμνασίου', '3η Γυμνασίου', '1η Λυκείου', '2η Λυκείου', '3η Λυκείου'][previousYearNum - 1] || `Year ${previousYearNum}`)
...
- Language: write questions and options in ${lang} (${locale === 'el-GR' ? 'Greek' : 'English'}).
...
Return a valid JSON object with this exact structure (no markdown, no code block):
{
   ...
   */
  private getFallbackDiagnosticQuestions(yearGroup: any, subjects: any[], locale: string = 'en-GB') {
    const allQuestions: any[] = [];

    for (const subject of subjects) {
      allQuestions.push(...this.getFallbackQuestionsForSubject(yearGroup, subject, locale));
    }

    if (allQuestions.length === 0) {
      this.logger.warn('No fallback questions matched subjects; using default diagnostic set so Part B can display.');
      return this.getDefaultFallbackDiagnosticQuestions(yearGroup, locale);
    }

    if (allQuestions.length < 30) {
      const defaultSet = this.getDefaultFallbackDiagnosticQuestions(yearGroup, locale);
      const existingIds = new Set(allQuestions.map((q) => q.id));
      const extra = defaultSet.questions
        .filter((q) => !existingIds.has(q.id))
        .slice(0, 30 - allQuestions.length);
      allQuestions.push(...extra);
      this.logger.log(`Padded fallback to ${allQuestions.length} questions (target 30).`);
    }

    return { questions: this.deduplicateQuestionsById(allQuestions).slice(0, 30) };
  }

  /** Drop duplicate question ids (keeps first occurrence). */
  private deduplicateQuestionsById(questions: any[]): any[] {
    const seen = new Set<string>();
    return questions.filter((q, index) => {
      const id = (q.id ?? `__missing_${index}`).toString();
      if (seen.has(id)) {
        return false;
      }
      seen.add(id);
  ...
  /** Stable per-subject prefix so fallback ids stay unique across many DB subjects. */
  private getSubjectQuestionIdPrefix(subject: any, normalized: string): string {
    const raw = (subject.id || subject.name || normalized || 'sub').toString();
   ...
   */
  private getFallbackQuestionsForSubject(yearGroup: any, subject: any, locale: string = 'en-GB') {
    const questions: any[] = [];
    const yearNum = this.getLogicalYearNumber(yearGroup);
    const name = (subject.name || '').toLowerCase().replace(/\s+/g, '_');
    const normalized =
      name === 'mathematics' || name === 'math' || name === 'mathimatika' ? 'maths'
      : name === 'greek_language' || name === 'greek' || name === 'greek_literature' || name === 'glossa' ? 'english'
      : name === 'physics' || name === 'chemistry' || name === 'biology' || name === 'fysiki' || name === 'epistimi' ? 'science'
      : name;

    const subjectKey = this.getSubjectQuestionIdPrefix(subject, normalized);
    const isEl = locale === 'el-GR';
    const subjDisplay = subject.displayName || subject.name || (isEl ? 'Μάθημα' : 'Subject');

    if (isEl && normalized === 'english') {
      questions.push(
        {
          id: `${subjectKey}_eng_q1`,
          subject: subject.name,
          subjectDisplay: subjDisplay,
          question: 'Ποιος είναι ο κύριος σκοπός μιας προτάσεως-θέματος σε μια παράγραφο;',
          difficulty: 'easy',
          type: 'multiple_choice',
          options: [
            { value: 'a', label: 'Να κλείσει η παράγραφο' },
            { value: 'b', label: 'Να παρουσιάσει την κύρια ιδέα' },
            { value: 'c', label: 'Να δώσει παραδείγματα' },
            { value: 'd', label: 'Να μπερδέψει τον αναγνώστη' },
          ],
          correctAnswer: 'b',
          skillTested: 'writing',
        },
        {
          id: `${subjectKey}_eng_q2`,
          subject: subject.name,
          subjectDisplay: subjDisplay,
          question: 'Ποιο από τα παρακάτω είναι παράδειγμα παρομοίωσης;',
          difficulty: 'medium',
          type: 'multiple_choice',
          options: [
            { value: 'a', label: 'Ο άνεμος ούρλιαζε στα δέντρα' },
            { value: 'b', label: 'Το χαμόγελό της ήταν σαν τον ήλιο' },
            { value: 'c', label: 'Είναι λιοντάρι στη μάχη' },
            { value: 'd', label: 'Τα φύλλα χόρευαν στον αέρα' },
          ],
          correctAnswer: 'b',
          skillTested: 'reading',
        },
    ...
    if (isEl && normalized === 'maths') {
      questions.push(
        {
          id: `${subjectKey}_maths_q1`,
          subject: subject.name,
          subjectDisplay: subjDisplay,
        ...
        {
          id: `${subjectKey}_maths_q2`,
          subject: subject.name,
          subjectDisplay: subjDisplay,
          question: 'Λύσε ως προς x: 2x + 5 = 13',
          difficulty: 'medium',
          type: 'multiple_choice',
          options: [
            { value: 'a', label: 'x = 3' },
    ...
    if (isEl && normalized === 'science') {
      questions.push(
        {
          id: `${subjectKey}_sci_q1`,
          subject: subject.name,
          subjectDisplay: subjDisplay,
          question: 'Ποιος είναι ο ρόλος του πυρήνα σε ένα κύτταρο;',
          difficulty: 'easy',
          type: 'multiple_choice',
          options: [
            { value: 'a', label: 'Να παράγει ενέργεια' },
            { value: 'b', label: 'Να ελέγχει το κύτταρο και να αποθηκεύει DNA' },
            { value: 'c', label: 'Να χωνεύει τροφή' },
            { value: 'd', label: 'Να προστατεύει το κύτταρο' },
          ],
          correctAnswer: 'b',
          skillTested: 'biology',
        },
        {
          id: `${subjectKey}_sci_q2`,
          subject: subject.name,
          subjectDisplay: subjDisplay,
          question: 'Ποια μετατροπή ενέργειας γίνεται σε μια φακός με μπαταρίες;',
          difficulty: 'medium',
          type: 'multiple_choice',
          options: [
            { value: 'a', label: 'Από φως σε χημική' },
            { value: 'b', label: 'Από χημική σε φως και θερμότητα' },
            { value: 'c', label: 'Από θερμότητα σε φως' },
            { value: 'd', label: 'Από ήχο σε φως' },
          ],
          correctAnswer: 'b',
          skillTested: 'physics',
        },
      ...
      questions.push(
        {
          ...
          subject: subject.name,
          subjectDisplay: subjDisplay,
          question: GREEK_DIAGNOSTIC_GENERIC.mostInteresting(subjDisplay),
          difficulty: 'easy',
          type: 'multiple_choice',
          options: GREEK_DIAGNOSTIC_GENERIC.optionsMostInteresting,
          correctAnswer: 'a',
          skillTested: 'general',
        },
        {
          ...
          subject: subject.name,
          subjectDisplay: subjDisplay,
          question: GREEK_DIAGNOSTIC_GENERIC.improveArea(subjDisplay),
          difficulty: 'easy',
          type: 'multiple_choice',
          options: GREEK_DIAGNOSTIC_GENERIC.optionsImproveArea,
          correctAnswer: 'd',
          skillTested: 'general',
        },
      );
   ...
   * Ensures Part B always has questions to show (e.g. for Greek Year 3 when no K3 folder exists).
   */
  private getDefaultFallbackDiagnosticQuestions(yearGroup: any, locale: string = 'en-GB'): { questions: any[] } {
    const yearNum = this.getLogicalYearNumber(yearGroup);
    if (locale === 'el-GR') {
      const questions = GREEK_DEFAULT_DIAGNOSTIC_BASE.slice(0, 30).map((item, index) => ({
        ...
        difficulty: 'medium',
        skillTested: 'general_knowledge',
        explanation: diagnosticExplanationElGR(yearNum),
      }));
      return { questions };
    }
    const base = [
  ...
  async submitDiagnosticTest(userId: string, yearGroupId: string, answers: Record<string, string>) {
    ...
    // NOTE: We no longer enforce a minimum passing threshold for onboarding.
    // The diagnostic test is informational and always treated as "passed" so
    // students can proceed regardless of score, while we still record gaps.
    const passed = true;
    ...
    // Always create skill gaps based on diagnostic results so we can personalize
    // learning even for students with low scores.
...
Be encouraging and focus on growth. If they scored below 50%, emphasize that this is a learning opportunity.`;
  ...
  async getUnifiedMandatoryTest(userId: string, currentYearGroupId: string, preferredLocale?: string) {
    ...
    // Clean up any existing diagnostic tests for minimum-year students (Year 5 en-GB, dimotiko_1 el-GR)
    ...
    if (!isMinimumYear) {
      ...
      questionsArray = this.deduplicateQuestionsById(questionsArray);

        if (locale === 'el-GR' && questionsArray.length > 0 && onboardingQuestionsLookEnglish(questionsArray)) {
          const previousYearNum = currentYearNum - 1;
          const yearLocaleWhere = await whereYearGroupLocale(this.prisma, locale, true);
          const testYearGroup = await this.prisma.yearGroup.findFirst({
            where: { orderIndex: previousYearNum, ...yearLocaleWhere } as any,
          });
          if (testYearGroup) {
            const subjects = await this.prisma.subject.findMany({
              where: { yearGroupId: testYearGroup.id, isActive: true },
            ...
            const greekFallback = this.getFallbackDiagnosticQuestions(testYearGroup, subjects, locale);
            questionsArray = greekFallback.questions;
            this.logger.log(`Part B: Replaced English stored questions with Greek fallback (${questionsArray.length} questions).`);
          }
        }
      ...
      partB = {
        ...
        title:
          locale === 'el-GR'
            ? 'Μέρος Β: Αξιολόγηση Γνώσεων (Προηγούμενη Τάξη)'
            : 'Part B: Knowledge Assessment (Previous Year)',
    ...
    const partATitle = locale === 'el-GR' ? 'Μέρος Α: Αξιολόγηση Τρόπου Μάθησης' : 'Part A: Learning Style Assessment';
    const partADescription = locale === 'el-GR' ? 'Βοηθήστε μας να καταλάβουμε πώς μαθαίνετε καλύτερα' : 'Help us understand how you learn best';

    let partAQuestions = personalityTest.questions as any;
    if (locale === 'el-GR' && partAQuestions?.sections?.length) {
      partAQuestions = {
        ...partAQuestions,
        sections: normalizeElGRQuestionnaireSections(partAQuestions.sections),
      };
    }
  ...
  /**
   * Preload Part B diagnostic question banks for all non-minimum year groups.
   * This is intended for ops warm-up (e.g. deploy time) so first students in a
   * newly active year do not pay cold-start generation cost.
   */
  async warmupPartBDiagnostics(locales?: string[]) {
    const yearGroups = await this.prisma.yearGroup.findMany({
      orderBy: [{ locale: 'asc' }, { orderIndex: 'asc' }],
    });

    const localeBuckets = new Map<string, any[]>();
    for (const yearGroup of yearGroups) {
      ...
      const list = localeBuckets.get(locale) || [];
      list.push(yearGroup);
      localeBuckets.set(locale, list);
    }
    ...
    const summary = {
      locales: [] as Array<{
        ...
        skipped: number;
        failed: number;
        details: Array<{ currentYear: string; testedYear?: string; questionCount?: number; status: 'warmed' | 'skipped' | 'failed'; reason?: string }>;
      }>,
      totals: {
        warmed: 0,
        skipped: 0,
    ...
    for (const [locale, groups] of localeBuckets.entries()) {
      const sorted = [...groups].sort((a: any, b: any) => (a.orderIndex ?? 0) - (b.orderIndex ?? 0));
      const minOrderIndex = sorted.length > 0 ? (sorted[0] as any).orderIndex : 0;
      ...
      const localeSummary = {
        ...
        skipped: 0,
        failed: 0,
        details: [] as Array<{ currentYear: string; testedYear?: string; questionCount?: number; status: 'warmed' | 'skipped' | 'failed'; reason?: string }>,
      };

      for (const currentYearGroup of sorted) {
        const currentOrder = (currentYearGroup as any).orderIndex ?? 0;
        if (currentOrder <= minOrderIndex) {
          localeSummary.skipped++;
          summary.totals.skipped++;
          localeSummary.details.push({
            currentYear: (currentYearGroup as any).displayName || (currentYearGroup as any).name || String(currentOrder),
            status: 'skipped',
            reason: 'minimum year (Part B not required)',
          });
          continue;
        }

        const testedOrder = currentOrder - 1;
        const testedYearGroup = sorted.find((yg: any) => (yg.orderIndex ?? 0) === testedOrder);
        if (!testedYearGroup) {
          localeSummary.failed++;
          summary.totals.failed++;
          localeSummary.details.push({
            currentYear: (currentYearGroup as any).displayName || (currentYearGroup as any).name || String(currentOrder),
            status: 'failed',
            reason: `previous year group not found for orderIndex=${testedOrder}`,
          });
          continue;
        }
        ...
        try {
          const subjects = await this.prisma.subject.findMany({
            where: { yearGroupId: testedYearGroup.id, isActive: true },
            include: { skills: true },
            orderBy: { orderIndex: 'asc' },
          });

          const generated = await this.generateDiagnosticQuestions(testedYearGroup as any, subjects as any, locale);
          const questionCount = Array.isArray((generated as any)?.questions)
            ? (generated as any).questions.length
            : 0;

          localeSummary.warmed++;
          summary.totals.warmed++;
          localeSummary.details.push({
            currentYear: (currentYearGroup as any).displayName || (currentYearGroup as any).name || String(currentOrder),
            testedYear: (testedYearGroup as any).displayName || (testedYearGroup as any).name || String(testedOrder),
            questionCount,
            status: 'warmed',
          });
        } catch (error: any) {
  ...
  async submitUnifiedMandatoryTest(
    ...
    // Submit Part A: Personality Test (always mark as complete)
    ...
    // Check if student is in minimum year
    const isMinimumYear = await this.isMinimumYearGroup(currentYearGroupId);
    
    let diagnosticResults = null;
    let partBPassed = true; // Default to true for minimum year students
    
    // Skip Part B for minimum year students (en-GB: Year 5, el-GR: dimotiko_1)
    if (!isMinimumYear && currentYearNum !== minYearNum) {
      // Submit Part B when answers provided; if missing (e.g. Part B had no questions for this year), allow completion without blocking
      if (partBAnswers && Object.keys(partBAnswers).length > 0) {
        const diagnosticTestResult = await this.submitDiagnosticTest(userId, currentYearGroupId, partBAnswers);
        diagnosticResults = diagnosticTestResult;
        partBPassed = diagnosticTestResult.passed || false;
      } else {
        this.logger.warn(`Part B answers not provided for year group ${currentYearGroupId}; completing onboarding without Part B (e.g. diagnostic not available for this year)`);
        partBPassed = false;
      }
    } else {
      this.logger.log(`Student is in Year ${currentYearNum} (minimum year), Part B diagnostic test is skipped`);
      if (partBAnswers && Object.keys(partBAnswers).length > 0) {
        this.logger.warn(`Year ${currentYearNum} student provided Part B answers, but Part B is skipped - ignoring answers`);
      }
    }

    const requiresPartB = !isMinimumYear && currentYearNum !== minYearNum;
    const partBSubmitted = !!partBAnswers && Object.keys(partBAnswers).length > 0;
    const onboardingSuccess = !requiresPartB || (partBSubmitted && partBPassed);
    // Mark onboarding complete only when required onboarding parts were actually submitted.
    await this.prisma.studentProfile.updateMany({
      where: { userId },
      data: { onboardingCompleted: onboardingSuccess },
    });

    let message: string;
    if (onboardingSuccess) {
      message = 'Test completed! Your curriculum-aligned learning setup is ready.';
    } else if (!partBSubmitted) {
      message = 'Part A and subject selection completed. Please complete Part B to finish onboarding.';
    } else {
      message = 'Part A completed! Please complete Part B to finish onboarding.';
    }
...
Return as JSON:
{
  ...
  async getOnboardingStatus(userId: string) {
    ...
    // For minimum year students, only Part A is required (en-GB: Year 5, el-GR: dimotiko_1)
    if (isMinimumYear || currentYearNum === minYearNum) {
