...
export class UsersService {
  ...
  async updateProfile(userId: string, updateProfileDto: UpdateProfileDto) {
    ...
    // Check if this is onboarding completion (transitioning from false to true)
    const isOnboardingCompletion = updateProfileDto.onboardingCompleted === true && 
      (!existingProfile || !existingProfile.onboardingCompleted);

    // Prepare data with proper relation handling
    const { yearGroupId, ...restData } = updateProfileDto;
    ...
    if (existingProfile) {
      ...
      profile = await this.prisma.studentProfile.create({
        data: {
          ...
          onboardingCompleted: restData.onboardingCompleted ?? false,
        },
        include: {
          yearGroup: true,
        },
    ...
    // If onboarding just completed, trigger automatic exercise generation and weekly plan
    // This runs in the background so it doesn't slow down the profile update response
    if (isOnboardingCompletion && profile.yearGroupId) {
      this.logger.log(`Onboarding completed for user ${userId}, triggering automatic setup`);
      
      // Run exercise generation and then weekly plan generation in background
      this.setupNewStudent(userId, profile.yearGroupId);
    }
