...
export class AutoExerciseGeneratorService {
  ...
  /**
   * AI models sometimes return only hints/clues or empty `question` fields.
   * Students need an explicit question string for every item.
   */
  private normalizeGeneratedActivityContent(content: any, locale: string): any {
    const isEl = locale === 'el-GR';
    const fallbackTask = (n: number) =>
      isEl
        ? `Άσκηση ${n}: Βασισμένος/η στις οδηγίες παραπάνω, γράψε την απάντησή σου αναλυτικά.`
        : `Exercise ${n}: Based on the instructions above, write your answer in full.`;
    ...
    if (items.length === 0 && Array.isArray(content.clues)) {
      items = content.clues.map((c: any, i: number) =>
        typeof c === 'string' ? { question: `${fallbackTask(i + 1)} «${c}»`, hint: c } : c,
      );
    }
    ...
    out.items = items.map((raw: any, index: number) => {
      ...
      if (!question || String(question).trim().length === 0) {
        if (hint != null && String(hint).trim().length > 0) {
          ...
          question = isEl
            ? `Άσκηση ${n}: Χρησιμοποίησε το παρακάτω βοήθημα και απάντησε με δικά σου λόγια.\n«${h}»`
            : `Exercise ${n}: Use the clue below and answer in your own words.\n"${h}"`;
        } else {
      ...
      return {
        ...
        ...(hint != null && String(hint).trim().length > 0 ? { hint: String(hint).trim() } : {}),
      };
    });
  ...
  private async generateActivityForSkill(params: {
    ...
    const languageBlock =
      locale === 'el-GR'
        ? `
LANGUAGE: Ο μαθητής διδάσκεται στα Ελληνικά. Γράψε ΟΛΟΚΛΗΡΗ τη δραστηριότητα στα Ελληνικά: τίτλος, περιγραφή, οδηγίες και κάθε ερώτηση.
ΣΗΜΑΝΤΙΚΟ: Κάθε αντικείμενο στον πίνακα "items" ΟΦΕΙΛΕΙ να έχει μη-κενό πεδίο "question" (η ακριβής απορία προς το μαθητή). Προαιρετικό "hint".
`
        : `
IMPORTANT: Every object in "items" MUST include a non-empty "question" field (the precise task the student answers). Optional "hint" is allowed.
`;
...
Generate appropriate content for ${subject.displayName} - ${skill.displayName}.`;
    ...
    try {
        ...
        messages: [
          {
            ...
            content:
              locale === 'el-GR'
                ? 'You are an expert curriculum designer for Greek schools. Always write ALL student-facing text in modern Greek (Ελληνικά). Never use English for titles, prompts, or questions unless quoting a proper noun.'
                : 'You are an expert curriculum designer creating engaging, age-appropriate learning activities.',
          },
          {
            role: 'user',
            content: prompt,
      ...
      const resources: any = {};
      if (result.content) {
        resources.content = this.normalizeGeneratedActivityContent(result.content, locale);
      }
...
CRITICAL JSON: Each object in "content.items" MUST include a non-empty string field "question" (the exact task). Optional "hint" only supplements the question — never replace the question with only a hint.
        ...
        messages: [
          {
            ...
            content:
              locale === 'el-GR'
                ? 'You are an expert curriculum designer for Greek schools. Always write ALL student-facing text in modern Greek (Ελληνικά). Every exercise item needs an explicit question string — not only hints.'
                : 'You are an expert curriculum designer creating engaging, age-appropriate learning activities. Always generate unique, varied problems - never repeat the same examples. Every item must include a non-empty "question" field.',
          },
          {
            role: 'user',
            content: prompt,
