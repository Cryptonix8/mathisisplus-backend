...
export class ActivityPdfParserService {
  ...
  async extractActivitiesWithVisionFromPdf(
    ...
    const isGreek = options?.locale === 'el-GR';
    const localeInstruction = isGreek
      ? '\n\nLANGUAGE: The curriculum may be in Greek. Extract activities in Greek when the source is Greek. Use LaTeX for math in any language.'
      : '';
